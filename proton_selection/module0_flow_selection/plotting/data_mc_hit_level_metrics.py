...
def main(file_dir, is_sim):
    ...
    for file in glob.glob(file_dir+'/*.h5'): # Loop over files files
        ...
        if is_sim:
            sel_truth = f['high_purity_sel']['protons']['sel_truth']['data']
        ...
        for event_id in sel_event_ids:
            ...
            ts_hits_data = charge_hits[charge_hit_ref]['ts']
            num_charge_hits = len(charge_hits_data)

            # Channel-level hit metrics
            iogroup_hits = charge_hits[charge_hit_ref]['iogroup']
            iochannel_hits = charge_hits[charge_hit_ref]['iochannel']
            chipid_hits = charge_hits[charge_hit_ref]['chipid']
            channelid_hits = charge_hits[charge_hit_ref]['channelid']
