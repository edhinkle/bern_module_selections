...
#         dqdx_smear = ndimage.uniform_filter(dqdx, 1, mode='nearest')
    ...
    def dx_estimate(profile_pos, hit_xyz, hit_idx, pixel_pitch, nsamples=10, tol=0.1):
        ...
        for iprof in range(profile_pos.shape[-2]):
            ...
            intersection = ProtonSelection.intersection(pos, dirs, c, n)
...
#         angle_term = -np.abs(angle) / np.pi
    ...
    def mean_neg_loglikelihood(r0, range_table, profile_n, profile_dqdx, profile_rr, profile_pos):
        ...
        pt_likelihood_dqdx, pt_likelihood_mcs = ProtonSelection.profile_likelihood(
    ...
    def run(self, source_name, source_slice, cache):
        ...
        if events.shape[0]:
            ...
            event_level_cuts_extended = np.array([np.full(max_tracks, event_level_cuts[i]) for i in range(len(event_level_cuts))])
            #print("Event level cuts extended shape:", event_level_cuts_extended.shape)

            all_initial_cuts = contained_in_fid & event_level_cuts_extended
